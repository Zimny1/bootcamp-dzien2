<template>
    <div>
        ...
        <div class="w-100 flex flex-row-reverse">
            <button @click="pobierzWpisy" class="float-right bg-blue-600 rounded-md text-white p-4">refresh</button>
        </div>
        <div class="grid mx-6 gap-4 my-4">
            <div v-for="wpis in wpisy" 
            class="drop-shadow-xl bg-stone-300">
                <p>{{ wpis }}</p>
            </div>
        </div>
        <div class="flex justify-center flex-col">
            <input v-model="nowyBlog" type="text" class="border-2 border-blue-600 p-4">
            <button @click="dodajWpisy" class="float-right bg-blue-600 rounded-md text-white p-4">dodaj</button>
        </div>
    </div>
</template>
